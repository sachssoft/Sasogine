...
public abstract class TileEditorRuntimeBase : RuntimeBase
{
    ...
    protected TileEditorRuntimeBase(OrthographicTileCamera camera, IEffectAdapter? effect) : base(camera, effect)
    {
    ...
    public override void Load(GameBaseContext context)
    {
        base.Load(context);
        _tile_primitive = new TilePrimitive();
        _cursor_tile = IGameApplication.Current.GraphicsDevice.CreateEmptyTexture(Color.White);
        UpdateGridFromMap();
    }
    ...
    protected override void OnSceneRender(GameFrameContext context)
    {
        base.OnSceneRender(context);
    ...
    private static BasicEffectAdapter CreateEffect() => new();

    private void HandleMoveCamera(GameFrameContext context)
    {
    ...
    private void HandleEditorZoomCamera(GameFrameContext context)
    {
        if (IsAnySurfaceHovered) return;
    ...
    private void HandleCursor(GameFrameContext context)
    {
    ...
    private void UpdateDiagnosticsDisplay(GameFrameContext context)
    {
    ...
    private void UpdateMap(GameFrameContext context)
    {
        if (Benchmark == null)
        {
            Map?.Update(context);
    ...
    private void DrawMap(TileRenderer renderer, GameFrameContext context, TileDrawingOptions options)
    {
        var view_bounds = TilingHelper.GetViewBounds(IGameApplication.Current.GraphicsDevice, Camera, TileSize);
