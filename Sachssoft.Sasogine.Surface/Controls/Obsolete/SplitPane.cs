...
public abstract class SplitPane : Container
{
    ...
    private void HandleOnPressedChanged(object sender, EventArgs args)
    {
        ...
        if (!handle.IsPressed)
        {
            ...
            UIEnvironment.SetMouseCursorFromWidget = true;
            UIEnvironment.MouseCursorType = new StyleProperty<MouseCursorType>(MouseCursor.Value ?? UIEnvironment.DefaultMouseCursorType);
        }
        else if (Desktop != null)
        {
            ...
            UIEnvironment.SetMouseCursorFromWidget = false;
            if (Orientation == Orientation.Horizontal)
            {
                UIEnvironment.MouseCursorType = MouseCursorType.SizeWE;
            }
            else
            {
                UIEnvironment.MouseCursorType = MouseCursorType.SizeNS;
            }
        }
    }
    ...
    public void Reset()
    {
        ...
        //else
        //{
        //    var asImage = HandleStyle.Background as RegionBrush;
        //    if (asImage != null)
        //    {
        //        handleSize = Orientation == Orientation.Horizontal
        //            ? asImage.Region.Size.X
        //            : asImage.Region.Size.Y;
        //    }
        //}
        ...
        foreach (var w in Widgets)
        {
            ...
            if (i > 0)
            {
                ...
                if (Orientation == Orientation.Horizontal)
                {
                    ...
                    handle.VerticalAlignment = Visuals.VerticalAlignment.Stretch;
                }
                else
                {
                    handle.MouseCursor = MouseCursorType.SizeNS;
                    handle.HorizontalAlignment = Visuals.HorizontalAlignment.Stretch;
                }
