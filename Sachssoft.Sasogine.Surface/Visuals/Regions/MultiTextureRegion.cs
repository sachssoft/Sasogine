...
namespace Sachssoft.Sasogine.Surface.Visuals.Regions
{
    ...
    public class MultiTextureRegion : ITextureRegion
    {
        ...
        /// </summary>
        private readonly List<(ITextureRegion image, Point offset, Color color)> _layers = new();

        /// <summary>
        /// Gets the list of layers as tuples containing:
        /// <c>(IImage image, Point offset, Color color)</c>.
        /// </summary>
        public IReadOnlyList<(ITextureRegion image, Point offset, Color color)> Layers => _layers;
        ...
        public void AddLayer(ITextureRegion image, Point? offset = null, Color? color = null)
        {
            if (image != null)
            {
                _layers.Add((
