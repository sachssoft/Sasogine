    ...
    public override void Draw(GameFrameContext context, TilePrimitive primitive, TileRenderer renderer, Texture2D cursor_texture)
    {
        ...
        for (int y = 0; y <= size.Y; y++)
        {
            for (int x = 0; x <= size.X; x++)
            {
                ...
                if (DrawMode == SelectionDrawMode.Filled)
                {
                    coord = new Coordinate(_scope.Lower.X + x, _scope.Lower.Y + y);
                }
                else /*if (DrawMode == SelectionDrawMode.Outline)*/
                {
                    bool is_border = x == 0 || x == size.X ||
                                     y == 0 || y == size.Y;
