...
[Obsolete("Replaced by ResourceStore or smilar")]
public partial class GameResourceManager
{
    ...
    public GameApplication GameApplication => _gameApplication;
    ...
    private TLoader CreateLoader<TLoader>(string path) where TLoader : LoaderBase
    {
    ...
    public static void RegisterType<TData>(
        Func<GameResourceManager, LoaderBase, TData>? syncFactory = null,
        Func<GameResourceManager, LoaderBase, Task<TData>>? asyncFactory = null)
    {
    ...
    public void Add(string key, string path, ResourceLoaderType type = ResourceLoaderType.Content)
    {
        Add<Stream>(key, path, null, type);
    }

    public void Add(string key, string path, CultureInfo? culture, ResourceLoaderType type = ResourceLoaderType.Content)
    {
        Add<Stream>(key, path, culture, type);
    }

    public void Add<TData>(string key, string path, ResourceLoaderType type = ResourceLoaderType.Content)
    {
        Add<TData>(key, path, null, type);
    }

    public void Add<TData>(string key, string path, CultureInfo? culture, ResourceLoaderType type = ResourceLoaderType.Content)
    {
        culture ??= CultureInfo.InvariantCulture;

        switch (type)
        {
            case ResourceLoaderType.Content:
                AddContent<TData>(key, path, culture);
                break;
            case ResourceLoaderType.EmbeddedResource:
                Add<TData, EmbeddedResourceLoader>(key, path, culture);
                break;
            case ResourceLoaderType.ExternalFile:
                Add<TData, LocalFileLoader>(key, path, culture);
                break;
            default:
                throw new InvalidOperationException($"Unsupported ResourceLoaderType: {type}");
        }
    }
    ...
    public void AddAsync(string key, string path, ResourceLoaderType type = ResourceLoaderType.Content)
    {
        AddAsync<Stream>(key, path, null, type);
    }

    public void AddAsync(string key, string path, CultureInfo? culture = null, ResourceLoaderType type = ResourceLoaderType.Content)
    {
        AddAsync<Stream>(key, path, culture, type);
    }

    public void AddAsync<TData>(string key, string path, ResourceLoaderType type = ResourceLoaderType.Content)
    {
        AddAsync<TData>(key, path, null, type);
    }

    public void AddAsync<TData>(string key, string path, CultureInfo? culture = null, ResourceLoaderType type = ResourceLoaderType.Content)
    {
        culture ??= CultureInfo.InvariantCulture;
        ...
        switch (type)
        {
            ...
            case ResourceLoaderType.EmbeddedResource:
                AddAsync<TData, EmbeddedResourceLoader>(key, path, culture);
                break;

            case ResourceLoaderType.ExternalFile:
                AddAsync<TData, LocalFileLoader>(key, path, culture);
                break;
    ...
    /// </summary>
    public TData AddAndLoad<TData>(string key, string path, CultureInfo? culture = null, ResourceLoaderType type = ResourceLoaderType.Content)
    {
    ...
    /// </summary>
    public async Task<TData?> AddAndLoadAsync<TData>(string key, string path, CultureInfo? culture = null, ResourceLoaderType type = ResourceLoaderType.Content)
    {
    ...
    {
        public Func<GameResourceManager, LoaderBase, TData>? SyncFactory { get; }
        public Func<GameResourceManager, LoaderBase, Task<TData>>? AsyncFactory { get; }

        public FactoryWrapper(
            Func<GameResourceManager, LoaderBase, TData>? syncFactory,
            Func<GameResourceManager, LoaderBase, Task<TData>>? asyncFactory)
        {
            SyncFactory = syncFactory;
            AsyncFactory = asyncFactory;
        }

        public TData CreateSync(GameResourceManager app, LoaderBase loader)
        {
            if (SyncFactory == null)
        ...
        public Task<TData> CreateAsync(GameResourceManager app, LoaderBase loader)
        {
