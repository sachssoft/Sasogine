...
public class MenuItem : BaseObject, IMenuItem, ICommandSource, ICheckable, IEnableable
{
    ...
    public bool IsEnabled
    {
        get
        {
            if (Command != null && !Command.CanExecute(ViewOwner, CommandParameter))
            {
                return false;
            }
            return ImageWidget.IsEnabled;
        }
        set
        {
            ImageWidget.IsEnabled = Label.IsEnabled = Shortcut.IsEnabled = value;
        }
    }
    ...
    public bool IsCheckable { get; set; } // Auto Checked
