...
public class EditorToolCursor
{
    ...
    public virtual void Update(TileEditorRuntimeBase runtime, GameContext context)
    {
        if (runtime?.Map == null)
            return;
    ...
    public virtual void Draw(GameContext context, TilePrimitive primitive, TileRenderer renderer, Texture2D cursor_texture)
    {
        if (!_is_cursor_inside)
            return;
