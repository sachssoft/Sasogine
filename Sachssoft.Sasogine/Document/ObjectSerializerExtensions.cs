...
public static class ObjectSerializerExtensions
{
    ...
    {
        var result = reader.ReadString(context, ColorUtils.ToHexString(fallback, alpha))!;
        return ColorUtils.TryParse(result, alpha, out var color) ? color : fallback;
    }
    ...
    {
        writer.WriteString(context, value.HasValue ? ColorUtils.ToHexString(value.GetValueOrDefault(), alpha) : null);
    }
