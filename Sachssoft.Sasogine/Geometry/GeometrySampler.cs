...
namespace Sachssoft.Sasogine.Geometry
{
    ...
    public static class GeometrySampler
    {
        ...
        public static Vector2[] SampleArc(Vector2 start, Vector2 end, float rx, float ry, float xAxisRotation, bool largeArc, bool sweep, int segments)
        {
            ...
            float numerator = float.Max(0f, rxSq * rySq - rxSq * y1pSq - rySq * x1pSq);
            float denominator = rxSq * y1pSq + rySq * x1pSq;
            float factor = (denominator == 0f) ? 0f : (largeArc == sweep ? -1f : 1f) * float.Sqrt(numerator / denominator);
            ...
            float theta1 = float.Atan2((y1p - cyp) / ry, (x1p - cxp) / rx);
            float deltaTheta = float.Atan2((-y1p - cyp) / ry, (-x1p - cxp) / rx) - theta1;
