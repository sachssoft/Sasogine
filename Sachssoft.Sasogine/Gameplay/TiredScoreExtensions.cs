...
namespace Sachssoft.Sasogine.Gameplay
{
    ...
    public static class TieredScoreExtensions
    {
        ...
        public static TieredScore<TValue> CoerceLow<TValue>(this TieredScore<TValue> baseValue)
            where TValue : struct, IComparable<TValue>
        {
            return new TieredScore<TValue>(baseValue.Bronze, baseValue.Silver, baseValue.Gold, ScoreDirection.Low);
        }

        public static TieredScore<TValue> CoerceHigh<TValue>(this TieredScore<TValue> baseValue)
            where TValue : struct, IComparable<TValue>
        {
            return new TieredScore<TValue>(baseValue.Bronze, baseValue.Silver, baseValue.Gold, ScoreDirection.High);
        }
        ...
        public static TValue Difference<TValue>(this TieredScore<TValue> score, TierResult tier, TValue value)
            where TValue : struct, IComparable<TValue>
        {
            var tierValue = GetTierValue(score, tier);
            if (typeof(TValue) == typeof(int)) return (TValue)(object)((int)(object)value - (int)(object)tierValue);
            if (typeof(TValue) == typeof(float)) return (TValue)(object)((float)(object)value - (float)(object)tierValue);
            if (typeof(TValue) == typeof(TimeSpan)) return (TValue)(object)((TimeSpan)(object)value - (TimeSpan)(object)tierValue);
        ...
        public static TValue AbsDifference<TValue>(this TieredScore<TValue> score, TierResult tier, TValue value)
            where TValue : struct, IComparable<TValue>
        {
            var diff = score.Difference(tier, value);
            if (typeof(TValue) == typeof(int)) return (TValue)(object)Math.Abs((int)(object)diff);
            if (typeof(TValue) == typeof(float)) return (TValue)(object)Math.Abs((float)(object)diff);
        ...
        public static float PercentOfTier<TValue>(this TieredScore<TValue> score, TierResult tier, TValue value)
            where TValue : struct, IComparable<TValue>
        {
            var diff = score.Difference(tier, value);
            var tierValue = GetTierValue(score, tier);
        ...
        public static TierResult ClosestTier<TValue>(this TieredScore<TValue> score, TValue value)
            where TValue : struct, IComparable<TValue>
        {
        ...
        private static TValue GetTierValue<TValue>(TieredScore<TValue> score, TierResult tier)
            where TValue : struct, IComparable<TValue>
        {
            return tier switch
            {
        ...
        public static bool IsBetter<TValue>(this TieredScore<TValue> score, TValue a, TValue b)
            where TValue : struct, IComparable<TValue> =>
            score.Direction == ScoreDirection.High ? a.CompareTo(b) > 0 : a.CompareTo(b) < 0;

        public static bool IsWorse<TValue>(this TieredScore<TValue> score, TValue a, TValue b)
            where TValue : struct, IComparable<TValue> =>
            score.Direction == ScoreDirection.High ? a.CompareTo(b) < 0 : a.CompareTo(b) > 0;

        public static bool Equals<TValue>(this TieredScore<TValue> score, TValue a, TValue b)
            where TValue : struct, IComparable<TValue> =>
            a.CompareTo(b) == 0;

        public static bool LessThan<TValue>(this TieredScore<TValue> score, TValue a, TValue b)
            where TValue : struct, IComparable<TValue> =>
            a.CompareTo(b) < 0;

        public static bool LessOrEqual<TValue>(this TieredScore<TValue> score, TValue a, TValue b)
            where TValue : struct, IComparable<TValue> =>
            a.CompareTo(b) <= 0;

        public static bool GreaterThan<TValue>(this TieredScore<TValue> score, TValue a, TValue b)
            where TValue : struct, IComparable<TValue> =>
            a.CompareTo(b) > 0;

        public static bool GreaterOrEqual<TValue>(this TieredScore<TValue> score, TValue a, TValue b)
            where TValue : struct, IComparable<TValue> =>
            a.CompareTo(b) >= 0;
