...
// By Tobias Sachs
public class WidgetCollection<TWidget> : ObservableCollection<TWidget> where TWidget : Widget
{
    ...
    public bool TryRemoveRange(IEnumerable<Widget> widgets)
    {
        ...
        {
            if (Remove((TWidget)widget)) // nutzt TryRemove intern
                any_removed = true;
        }
    ...
    public bool TryMoveRange(int old_index, int new_index, int count)
    {
        ...
        {
            Insert(new_index + i, (TWidget)items[i]);
        }
