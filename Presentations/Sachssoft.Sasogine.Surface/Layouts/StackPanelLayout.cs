...
public class StackPanelLayout : ILayout
{
    ...
    {
        var i = 0;
        if (Proportions.Count != widgets.Count()) // Anzahl stimmt nicht → Reset
        {
            Proportions.Clear();
        }
        ...
        foreach (var widget in widgets)
        {
            ...
            if (i < Proportions.Count)
                Proportions[i] = new Proportion(StackPanel.GetProportionType(widget), StackPanel.GetProportionValue(widget));
            else
                Proportions.Add(new Proportion(StackPanel.GetProportionType(widget), StackPanel.GetProportionValue(widget)));
