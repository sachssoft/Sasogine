...
namespace Sachssoft.Sasogine.Input
{
    ...
    /// </typeparam>
    public abstract class InputInteractionManager<TButton>
        where TButton : unmanaged, Enum
    {
        private static readonly TButton[] _allButtons =
            Enum.GetValues<TButton>();
        ...
        /// </exception>
        protected InputInteractionManager(IInputState<TButton> initialState)
        {
        ...
        /// <summary>
        /// Gets or sets the listener that receives input interaction notifications.
        /// </summary>
        public IInputInteractionListener? Listener { get; set; }

        /// <summary>
        /// Adds a button combination with a press action and an optional release action.
        /// </summary>
        /// <param name="buttons">The buttons that must be pressed simultaneously.</param>
        /// <param name="pressAction">The action invoked when the combination becomes pressed.</param>
        /// <param name="releaseAction">The optional action invoked when the combination is released.</param>
        ...
        /// <param name="pressAction">The action invoked when the button becomes pressed.</param>
        /// <param name="releaseAction">The optional action invoked when the button is released.</param>
        public void Add(
            TButton button,
            Action pressAction,
            Action? releaseAction = null)
        ...
        /// </summary>
        /// <param name="buttons">The buttons identifying the combinations to remove.</param>
        ...
        /// </summary>
        /// <param name="sequence">The buttons that must be pressed in the specified order.</param>
        /// <param name="action">The action invoked when the sequence is completed.</param>
        /// <param name="timeout">
        ...
        /// </summary>
        /// <param name="sequence">The button sequence to remove.</param>
        public void RemoveSequence(IList<TButton> sequence)
        {
            ArgumentNullException.ThrowIfNull(sequence);

            _sequences.RemoveAll(
                entry => SequenceEquals(entry.Sequence, sequence));
        }
        ...
        public void ClearActiveCombinations(bool triggerReleaseActions = true)
        {
            if (triggerReleaseActions)
            {
                foreach (var entry in _activeCombos)
                {
                    ...
                    if (Listener != null)
                        Listener.Release(typeof(TButton), ToInteractions(entry.Buttons));
                }
            }
        ...
        /// </summary>
        /// <param name="buttons">The buttons identifying the active combination.</param>
        ...
        /// <summary>
        /// Updates the interaction manager using the current scene update context.
        /// </summary>
        /// <param name="context">
        ...
        /// <summary>
        /// Updates the registered interactions using the specified input state.
        /// </summary>
        /// <param name="currentState">The current input state.</param>
            ...
            TimeSpan elapsed)
        {
            ...
            {
                var isPressedNow = true;
                var wasPressedBefore = true;
                ...
                if (isPressedNow && !wasPressedBefore)
                {
                    ...
                    if (Listener != null)
                        Listener.Press(typeof(TButton), ToInteractions(entry.Buttons));
        ...
        private static ulong[] ToInteractions(HashSet<TButton> buttons)
        {
            var interactions = new ulong[buttons.Count];
            var index = 0;

            foreach (var button in buttons)
                interactions[index++] = EnumInteractionConverter<TButton>.ToUInt64(button);

            return interactions;
        }

        private static ulong[] ToInteractions(IList<TButton> buttons)
        {
            var interactions = new ulong[buttons.Count];

            for (int i = 0; i < buttons.Count; i++)
                interactions[i] = EnumInteractionConverter<TButton>.ToUInt64(buttons[i]);
        ...
        private static bool SequenceEquals(
            IList<TButton> first,
            IList<TButton> second)
        {
            if (first.Count != second.Count)
                return false;

            var comparer = EqualityComparer<TButton>.Default;
        ...
        protected class SequenceEntry
        {
            ...
            public void Update(
                ...
                TimeSpan elapsed)
            {
                Update(current, previous, elapsed, null);
            }
            ...
            public void Update(
                ...
                IInputState<TButton> previous,
                TimeSpan elapsed,
                IInputInteractionListener? listener)
            {
                ...
                var buttonToMatch = Sequence[_currentIndex];
                    ...
                    previous.IsButtonUp(buttonToMatch))
                {
                    ...
                    if (_currentIndex == Sequence.Count)
                    {
                        ...
                        if (listener != null)
                            listener.Trigger(typeof(TButton), ToInteractions(Sequence));
                ...
                TButton exclude)
            {
                ...
                {
                    if (comparer.Equals(button, exclude))
                        continue;
