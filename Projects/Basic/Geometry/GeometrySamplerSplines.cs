...
namespace Sachssoft.Sasogine.Geometry
{
    ...
    public static partial class GeometrySampler
    {
        ...
        /// otherwise, <see langword="false"/>.
        /// </param>
        /// <returns>The sampled points of the spline.</returns>
        public static Point2[] SampleCatmullRom(
            Point2[] points,
            int segmentsPerSpan,
            bool closed = false)
        {
            ...
            int spanCount = closed
                ? points.Length
                : points.Length - 1;

            Point2[] result = new Point2[spanCount * segmentsPerSpan + 1];
            int resultIndex = 0;

            for (int span = 0; span < spanCount; span++)
            {
                Point2 p0 = GetCatmullRomPoint(points, span - 1, closed);
                Point2 p1 = GetCatmullRomPoint(points, span, closed);
                Point2 p2 = GetCatmullRomPoint(points, span + 1, closed);
                Point2 p3 = GetCatmullRomPoint(points, span + 2, closed);

                for (int i = 0; i < segmentsPerSpan; i++)
                {
                    float t = i / (float)segmentsPerSpan;
                    result[resultIndex++] = EvaluateCatmullRom(p0, p1, p2, p3, t);
                }
            }

            result[resultIndex] = closed
                ? points[0]
                : points[points.Length - 1];
        ...
        /// Gets a control point used by a Catmull-Rom span.
        /// </summary>
        private static Point2 GetCatmullRomPoint(
            Point2[] points,
            int index,
            bool closed)
        {
        ...
        /// Evaluates a single Catmull-Rom spline span.
        /// </summary>
        private static Point2 EvaluateCatmullRom(
            Point2 p0,
            Point2 p1,
            Point2 p2,
            Point2 p3,
        ...
        /// Samples a clamped uniform B-spline.
        /// </summary>
        /// <param name="controlPoints">The control points of the spline.</param>
        /// <param name="degree">
        /// The polynomial degree of the spline.
        /// A degree of 3 creates a cubic B-spline.
        /// </param>
        /// <param name="segments">The total number of sampled segments.</param>
        /// <returns>The sampled points of the B-spline.</returns>
        public static Point2[] SampleBSpline(
            ...
            int segments)
        {
            ...
            double[] knots = CreateUniformClampedKnots(
                controlPoints.Length,
                degree);

            Point2[] result = new Point2[segments + 1];

            double start = knots[degree];
            double end = knots[controlPoints.Length];

            for (int i = 0; i <= segments; i++)
            {
                double t = start +
                    (end - start) * (i / (double)segments);

                result[i] = EvaluateBSpline(
                    controlPoints,
                    degree,
                    knots,
                    t);
        ...
        /// Evaluates a B-spline using the de Boor algorithm.
        /// </summary>
        private static Point2 EvaluateBSpline(
            Point2[] controlPoints,
            int degree,
            double[] knots,
            double t)
        {
            ...
            Point2[] local = new Point2[degree + 1];

            for (int j = 0; j <= degree; j++)
                local[j] = controlPoints[span - degree + j];
            ...
            for (int r = 1; r <= degree; r++)
            {
                for (int j = degree; j >= r; j--)
                {
                    ...
                    double denominator =
                        knots[index + degree - r + 1] -
                        knots[index];

                    float alpha = denominator == 0d
                        ? 0f
                        : (float)((t - knots[index]) / denominator);

                    Point2 previous = local[j - 1];
                    Point2 current = local[j];
        ...
        /// <param name="startTangent">The tangent at the start position.</param>
        /// <param name="end">The end position.</param>
        /// <param name="endTangent">The tangent at the end position.</param>
        /// <param name="segments">The number of sampled segments.</param>
        /// <returns>The sampled points of the Hermite curve.</returns>
        public static Point2[] SampleHermite(
            Point2 start,
            Vector2 startTangent,
            Point2 end,
            Vector2 endTangent,
