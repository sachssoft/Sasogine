...
public sealed class PathBuilder
{
    ...
    private readonly List<Point2[]> _polygons = new();
    private readonly List<PathCommand> _commands = new();

    private Point2 _currentPoint;
    private Point2? _previousControlPoint;
    ...
    /// </exception>
    public PathBuilder Start(Point2 start)
    {
        if (_started)
        {
    ...
    /// <returns>This builder.</returns>
    public PathBuilder AddLine(Point2 point)
    {
        return AddLine(
            point.X,
            point.Y);
    ...
    public IReadOnlyList<IReadOnlyList<Point2>> Build()
    {
        var polygons =
            new List<IReadOnlyList<Point2>>(
                _polygons.Count +
                (_points.Count >= 3 ? 1 : 0));
    ...
    private void AddSampledPoints(
        IReadOnlyList<Point2> sampled)
    {
