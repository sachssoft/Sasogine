...
namespace Sachssoft.Sasogine.Geometry
{
    ...
    public static class PathTools
    {
        ...
        /// <param name="smoothFactor">The interpolation factor used to determine the smoothing control points.</param>
        /// <param name="maxAngleDeg">The maximum angle in degrees for a point to be smoothed.</param>
        /// <param name="segments">The number of generated segments for each smoothed corner.</param>
            ...
            for (int i = 1; i < points.Count - 1; i++)
            {
                ...
                if (angleDeg < maxAngleDeg)
                {
                    ...
                    for (int s = 0; s <= segments; s++)
                    {
                        ...
                        Vector2 pt = (1 - t) * (1 - t) * p1 + 2 * (1 - t) * t * current + t * t * p2;
                        result.Add(pt);
                    }
                }
                else
        ...
        /// </summary>
        /// <param name="points">The points that define the path.</param>
        /// <param name="tolerance">The maximum allowed deviation from the simplified path.</param>
        ...
        /// </summary>
        /// <param name="points">The points that define the path.</param>
        /// <param name="startIndex">The index of the first point of the segment.</param>
        /// <param name="endIndex">The index of the last point of the segment.</param>
        /// <param name="newPointCount">The number of additional points to generate.</param>
        /// <returns>A new list containing the resampled path.</returns>
        /// <exception cref="ArgumentOutOfRangeException">
        /// Thrown when the specified indices are outside the valid range or the start index is not before the end index.
        /// </exception>
        ...
        /// </summary>
        /// <param name="points">The points that define the path.</param>
        /// <param name="cornerIndex">The index of the corner to round.</param>
        /// <param name="radius">The radius of the rounded corner.</param>
        /// <param name="segments">The number of segments used to approximate the arc.</param>
        /// <returns>A new list containing the path with the specified corner rounded.</returns>
        /// <exception cref="ArgumentException">Thrown when the path contains fewer than three points.</exception>
        /// <exception cref="ArgumentOutOfRangeException">Thrown when the corner index refers to the first or last point.</exception>
        public static List<Vector2> RoundCorner(List<Vector2> points, int cornerIndex, float radius, int segments = 6)
        {
            ...
            for (int i = 0; i <= segments; i++)
            {
                ...
                result.Add(new Vector2(center.X + radius * MathF.Cos(angle), center.Y + radius * MathF.Sin(angle)));
            }
        ...
        /// <param name="maxAngleDeg">The maximum corner angle in degrees for automatic rounding.</param>
        /// <param name="segments">The number of segments used to approximate each rounded corner.</param>
        /// <returns>A new list containing the path with eligible corners rounded.</returns>
        /// <exception cref="ArgumentException">Thrown when the path contains fewer than three points.</exception>
        ...
        /// </summary>
        /// <param name="points">The points that define the path.</param>
        /// <param name="distance">The distance from the beginning of the path.</param>
        /// <returns>The interpolated point at the specified distance, or the last point when the distance exceeds the path length.</returns>
