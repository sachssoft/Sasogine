...
/// <summary>
/// Provides utility methods for resizing two-dimensional textures using
/// different image scaling algorithms.
/// </summary>
public static class Texture2DScaler
{
    /// <summary>
    /// Reduces the texture dimensions by half using a box filter.
    /// </summary>
    /// <remarks>
    /// Each destination pixel is calculated as the average of a 2x2 block
    /// of source pixels. This method is suitable for generating lower-resolution
    /// texture levels such as mipmaps.
    /// </remarks>
    /// <param name="device">The graphics device used to create the resulting texture.</param>
    /// <param name="source">The source texture to downscale.</param>
    /// <returns>
    /// A new texture with half the width and height of the source texture.
    /// If either resulting dimension would be zero, the original texture is returned.
    /// </returns>
    ...
    /// <summary>
    /// Resizes the source texture to the specified dimensions using
    /// bilinear interpolation.
    /// </summary>
    /// <remarks>
    /// Bilinear interpolation combines the four neighboring source pixels
    /// for each destination pixel, providing smoother results than simple
    /// nearest-neighbor sampling.
    /// </remarks>
    /// <param name="device">The graphics device used to create the resulting texture.</param>
    /// <param name="source">The source texture to resize.</param>
    /// <param name="targetWidth">The width of the resulting texture in pixels.</param>
    /// <param name="targetHeight">The height of the resulting texture in pixels.</param>
    /// <returns>
    /// A new texture with the specified dimensions containing the
    /// bilinearly interpolated source image.
    /// </returns>
    public static Texture2D DownscaleBilinear(
