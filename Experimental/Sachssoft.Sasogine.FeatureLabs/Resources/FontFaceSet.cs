using Sachssoft.Sasogine.Graphics.Fonts;
using System;
    ...
    public sealed class FontFaceSet : IResourceContainer
    {
        ...
        public FontFaceSet(string name, FontFace[] faces)
        {
            ...
            if (faces == null || faces.Length == 0)
                throw new ArgumentException("FontFamily must have at least one FontFace.");

            // Deduplicate faces by Weight + Style
            foreach (var face in faces)
            {
                if (_faces.Any(f => f.Weight == face.Weight && f.Style == face.Style))
                    continue;
                _faces.Add(face);
            }
            ...
            _faces.Sort((a, b) =>
            {
                int cmp = ((int)a.Weight).CompareTo((int)b.Weight);
                return cmp != 0 ? cmp : a.Style.CompareTo(b.Style);
            });

            // Build dictionary for fast lookup
            _lookup = _faces.ToDictionary(f => new FontKey(f.Weight, f.Style), f => f);
        }
